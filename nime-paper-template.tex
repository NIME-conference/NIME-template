...
\renewcommand{\shortauthors}{al-{\d S}{\=a}l{\=\i}{\d h}, Ben-D\'av{\'\i}d et al.}
...
%% page. Suggested dimensions: 1200x400px
\begin{teaserfigure}
  \includegraphics[width=\textwidth]{images/sampleteaser.png}
  \caption{Entangled meshwork of colorful banana patch cables. Image by WikiProfPC (CC BY-SA 4.0)}
...
The ``\verb|nimeart|'' document class can be used to prepare articles
...
The \texttt{nimeart} document class is used to prepare submissions to the
International Conference on New Interfaces for Musical Expression (NIME),
including double-anonymous initial submissions and final camera-ready
...
This document explains the main features of the \texttt{nimeart} document
class and how to configure it correctly for NIME submissions.

\subsection{Template Styles}

The primary option passed to the \texttt{nimeart} document class is the
...
\begin{verbatim}
\documentclass[STYLE]{nimeart}
\end{verbatim}

For NIME submissions, the selected template style determines aspects such
...
The ``\verb|nimeart|'' document class requires the use of the
...
\section{Anonymisation and Blind Review}
% Anonymisation is a \textbf{crucial} component for NIME paper submissions. We kindly ask authors to ensure that all information that could introduce bias in the review process is anonymised.

NIME review for the paper and music tracks are \textbf{double blind}; i.e., the reviewers don’t know who the authors are and the authors don’t know who the reviewers are. 
To enable this, we ask authors to ensure that all information that could introduce bias in the review process is anonymised.
All paper category submissions must be anonymised so that no description or information that can reveal the authors’ identities is included in the submission (e.g., detailed descriptions of locations, projects names, linked websites, images with faces of authors). Citations to the authors’ previous work should preferably be cited in the third person to enable proper review (e.g., “As described in our previous work [4]” should be “As described by Martin et al. [4]). 

If this is not possible, specific references may be anonymised like this:

[1] Anonymised Author A and Anonymised Author B. YYYY. ANONYMISED: a Software Package for Technical Computations, version N. Retrieved Month DD, YYYY from \url{http://anonymised.edu/path/index.html}


To anonymise author names, please specify \verb|anonymous,review| in square brackets when submitting your paper.
\begin{verbatim}
  \documentclass[sigconf,anonymous,review]{nimeart}
\end{verbatim}

You can hide text in the paper like this:

\begin{verbatim}
This work was performed at \anon{NSA}.
This work was performed at \anon[No Such Agency]{NSA}.
\end{verbatim}
which will appear in your paper's review version as:\\
"This work was performed at \anon{NSA}." and This work was performed at "\anon[No Such Agency]{NSA}."


If you want to anonymize any text by suppressing it, you can use:
\begin{verbatim}
  \begin{anonsuppress}
    This will not appear in your paper
    \cite{doe2023, wizard2022}.
  \end{anonsuppress}
\end{verbatim}
\begin{anonsuppress}
    This will not appear in your paper
...
For more information on anonymisation, you can checkout:
\url{https://nime2026.org/submissions/\#anonymisation}.
...
The ``\verb|nimeart|'' document class includes the ``\verb|booktabs|''
